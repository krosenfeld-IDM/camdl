...
runtime, etc.), also read `CLAUDE.md`. This file is for agents _using_ camdl —
...
- **Bumping `ir/VERSION` or editing `ocaml/lib/ir/serde.ml` /
  `rust/crates/ir/src/`.** IR schema changes break every golden file and require
  atomic OCaml + Rust
  - golden-regen commits. Per CLAUDE.md, these are explicit human-loop changes.
- **Editing inference math files** (`pgas.rs`, `pgas_grad.rs`, `nuts.rs`,
  `if2.rs`, `obs_loglik.rs`, `obs_model.rs`, `particle_filter.rs`). Per
  CLAUDE.md these are flagged as high-risk regardless of how mechanical the edit
  looks; ask before touching.
