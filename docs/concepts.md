...
*why* — the identifiability logic, what priors are actually for, and the
...
Fitting synthetic data generated from *known* truth tests the **pipeline**: did
...
So recovery is a gate you must pass *before* real data, but passing it proves
only that the machinery works. Whether the model is *right* is a separate
...
along a **ridge** — a small *late* introduction and a large *early* one produce
...
likelihood, a posterior that pins to a bound, or chains that disagree
(`R̂ ✗`). The diagnostics in `camdl docs workflow` are how you detect it; the
fix is below.

## Why priors are load-bearing, not a nuisance

A prior is how you supply the information the *data* lacks.
...
- On a **non-identified direction** (the ridge), the prior is what regularizes
  ...
  legitimate, *as long as* the prior is honest (grounded in knowledge, not data
...
prior chosen *to make the sampler converge* launders your ignorance into a
false-confident answer. camdl refuses silent flat priors for exactly this
reason — the prior is a scientific claim, so it must be explicit and defensible,
not a knob you turn until the chains agree.
...
2. The instinct — "fix the convergence," run more sweeps, loosen the threshold —
   ...
   converge *to*.
3. The fix was to add **structure** (weakly-informative priors), after which
   `R̂` passed *honestly* — because the posterior was now actually well-defined
   and the ridge had collapsed to a bowl.
...
*appearance* of a good answer.** For software whose outputs inform public-health
...
and therefore the estimates. The subtlety: synthetic recovery at the *same*
...
well-defined. They do **not** say the model is *right*. Three real-data checks,
in increasing strength:

- **Prior-predictive** — do the priors, *before* seeing data, imply
...
- **External oracle** — the strongest test: agreement with an *independent*
