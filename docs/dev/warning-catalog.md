...
| E291      | semantic       | a `scenarios { }` preset named `as_fitted` — reserved for the no-overlay row (the fitted model, no scenario applied) in the `scenario` column of `camdl fit predict` output; rename the scenario (2026-06-27 scenario-aware fit predict)                                                                                          |
