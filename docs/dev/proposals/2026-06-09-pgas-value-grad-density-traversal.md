...
- **Status:** Draft **v2** — reworked after an adversarial design review
  (2026-06-09, _needs-rework_) that found a direction error + the wrong seam
  (changelog at end). High-risk inference math (`CLAUDE.md`):
  `pgas.rs`/`pgas_grad.rs`/`chain_binomial.rs` — the inference owner's lane.
  This doc designs; it does not unilaterally implement.
- **Issues:** #197, #200, #79; retires the #20/#76 one-off-patch cycle. RC1 in
